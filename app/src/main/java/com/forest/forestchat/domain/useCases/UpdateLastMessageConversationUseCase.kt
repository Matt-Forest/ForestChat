...
@Singleton
class UpdateLastMessageConversationUseCase @Inject constructor(
    private val conversationDao: ConversationDao,
    private val messageDao: MessageDao
) {

    suspend operator fun invoke(conversation: Conversation) {
        messageDao.getAllByThreadId(conversation.id)
            ?.maxByOrNull { it.date }
            ?.let { message ->
                conversationDao.insert(conversation.copy(lastMessage = message))
            }
    }
